...
import org.bit.big.brother.m.k.cap.t.model.Point;
import org.bit.big.brother.m.k.cap.t.model.SubRect;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

public class ColorFillSeg {
	
	private static Logger logger = LoggerFactory.getLogger(ColorFillSeg.class);
	/**
	 * cfs进行分割,返回分割后的数组
	 * @param sourceImage
	 * @return
	...
	public static List<BinaryMatrix> cfs(BinaryMatrix im){
		...
		for (SubRect r : subImgList) {
			if (r.getWidth() < Constants.MIN_RECT_WIDTH || r.getHeight() < Constants.MIN_RECT_HEGITH) {
				logger.error("ColorFillSeg:丢弃小分割：([{}], [{}]) < ([{}], [{}])", r.getWidth(), 
			...
			if (sumOfBlack < Constants.MIN_RECT_PIXELS) {
				logger.error("ColorFillSeg:丢弃小分割：[{}] < [{}] (pixels)\n", sumOfBlack, Constants.MIN_RECT_PIXELS);
				continue;
			}

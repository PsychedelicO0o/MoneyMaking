package org.bit.big.brother.m.k.cap.t.model;
...
public final class BinaryMatrix {
	...
	private static Logger logger = LoggerFactory.getLogger(BinaryMatrix.class);
	...
	public BinaryMatrix trim() {
		...
		if (w <= Constants.MIN_RECT_WIDTH || h <= Constants.MIN_RECT_HEGITH) {
			logger.error("1.BinaryMatrix#trim:丢弃小分割：([{}], [{}]) < ([{}], [{}])", w, 
		...
		if (width <= Constants.MIN_RECT_WIDTH || height <= Constants.MIN_RECT_HEGITH) {
			logger.error("BinaryMatrix#trim:丢弃小分割：([{}], [{}]) < ([{}], [{}])", width, 
		...
		if (PointCount < Constants.MIN_RECT_PIXELS) {
			//丢弃像素点小于100的图片（噪点）
			logger.error("BinaryMatrix#trim:丢弃小分割：[{}] < [{}] (pixels)", PointCount, Constants.MIN_RECT_PIXELS);
			return null;
		}
		return new BinaryMatrix(res);
	}
